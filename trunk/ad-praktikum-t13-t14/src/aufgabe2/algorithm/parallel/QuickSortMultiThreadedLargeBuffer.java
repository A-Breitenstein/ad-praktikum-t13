 ...
 * Time: 17:15
 */
public class QuickSortMultiThreadedLargeBuffer {
    private LargeIntBuffer data;
    private ExecutorService threadPool;
    private long startLinks,startRechts;
    private int depth = 0;
    ...
    private QuickSortMultiThreadedLargeBuffer(LargeIntBuffer data, long startLinks, long startRechts, ExecutorService threadPool) {
        this.data = data;
        this.threadPool = threadPool;
        this.startLinks = startLinks;
        this.startRechts = startRechts;
    }
    public static boolean sort(LargeIntBuffer data, long startLinks, long startRechts,ExecutorService threadPool){
        QuickSortMultiThreadedLargeBuffer tmp = new QuickSortMultiThreadedLargeBuffer(data, startLinks, startRechts, threadPool);
        return tmp.start();
...
//            future = threadPool.submit(new DualPivotQuicksort(startLinks,startRechts));
    ...
    class quickSort implements Callable {
        private long links, rechts;
        quickSort( long links, long rechts) {
            this.links = links;
            this.rechts = rechts;
...
//                threadPool.submit(new InsertionSort(links,rechts));
     ...
     * @return
     */
    public static void blockSort_quick_singleThreaded(LargeIntBuffer data, long links, long rechts) {
        if (rechts - links < insertion_sort_grenze) {
            blockSort_insertion(data, links, rechts);
        } else {
            long positionPivot = quickSwap(data, links, rechts);
            blockSort_quick_singleThreaded(data, links, positionPivot - 1l);
            blockSort_quick_singleThreaded(data, positionPivot + 1l, rechts);
        }
    }
     ...
     * @param rechts die rechte Grenze (einschließlich), bis zu welcher sortiert werden soll
     */
    public static void blockSort_insertion(LargeIntBuffer data, long links, long rechts) {
        for (long i = links + 1l; i <= rechts; i++) {
            long j = i;
            int itemToSort = data.get(i);
            while (j > 0l && data.get(j - 1l) > itemToSort) {
                // insert
                data.put(j, data.get(j - 1l));
                j = j - 1l;
            }
            data.put(j, itemToSort);
        }
...
//            blockSort_quick(data, positionPivot + 1, rechts);
     ...
     */
    public static long quickSwap(LargeIntBuffer data, long links, long rechts) {

        long i = links;
        long j = rechts - 1l; // Starte mit j links vom Pivotelement
...
//	  }
     ...
     */
    private static void swap(LargeIntBuffer data, long pos1,long pos2) {
    ...
    public static void dualPivotQuicksortSingleThreaded(LargeIntBuffer data,int left,int right){
        int len = right - left;
        int x;
        if (len < INSERTIONSORT_GRENZE) { // insertion sort on tiny array
            blockSort_insertion(data,left,right);
            return;
        }
